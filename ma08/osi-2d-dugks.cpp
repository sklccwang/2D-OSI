...
#elif VV==2
const int Qx=64;        // discrete velocity in x-direction
const int Qy=64;        // discrete velocity in y-direction
#elif VV==3
const int Qx=80;        // discrete velocity in x-direction
const int Qy=80;        // discrete velocity in y-direction
#else
#endif
/*-----------------------------------------------*/
...
void setVelocity()
{
    ...
    dh=(b-a)/n;
    cmax=Vmax;
    double Co_X[np],Co_W[np];

    for(k=0;k<np;k++) Co_X[k]=Vmin+k*dh/4;//Co_X[k]=(k-0.5*n1)/n1*(b-a);
    ...
    for(k=0;k<Qx;k++) 
    {
        ex[k]=Co_X[k]; tpx[k]=Co_W[k];
        ey[k]=Co_X[k]; tpy[k]=Co_W[k];
        // printf("k=%3d ex=%.8e tp=%.8e \n ",k,ex[k],tpx[k]);
    }
}
#else
#endif
